  ...
  if [[ $OS == linux ]]; then
    ...
    case $DISTRO in
      ubuntu|debian)
        ...
        # Install required packages (without Mosquitto)
        print_msg "Installing required packages..." "$BLUE"
        DEBIAN_FRONTEND=noninteractive apt-get install -y git curl build-essential "$PYTHON" "$PYTHON"-venv pipx uv || true
        ;;
      arch)
        pacman -Sy --noconfirm git curl base-devel "$PYTHON" python-virtualenv python-pipx uv || true
        ;;
      *)
        print_msg "Unknown distro – please install git, curl, python3, pipx, uv manually." "$YELLOW"
        ;;
    esac
  else
    ...
    brew install git python3 pipx uv || true
...
EOF
      ...
      if [[ ! -f "$passwd_file" ]]; then
        ...
        if [[ -n "$mqtt_user" ]]; then
          ...
          # Offer both interactive and non-interactive options
          if ask_yn "Would you like to enter the password non-interactively (less secure but more reliable)?" false; then
            ...
            if [[ -n "$mqtt_pass" ]]; then
              ...
              # Create password file non-interactively with -b flag
              print_msg "Creating password file non-interactively..." "$BLUE"
              require_root mosquitto_passwd -c -b "$passwd_file" "$mqtt_user" "$mqtt_pass"
              ...
              if [[ -f "$passwd_file" && -s "$passwd_file" ]]; then
                ...
                print_msg "Failed to create password file non-interactively." "$RED"
              fi
            else
              print_msg "Empty password provided, cannot proceed with non-interactive mode." "$RED"
            fi
          else
            # Interactive password creation (original method)
          ...
          if [[ -n "$mqtt_user" ]]; then
            ...
            if ask_yn "Would you like to enter the password non-interactively (less secure but more reliable)?" false; then
              ...
              if [[ -n "$mqtt_pass" ]]; then
                ...
                # Add user non-interactively with -b flag
                print_msg "Adding user non-interactively..." "$BLUE"
                require_root mosquitto_passwd -b "$passwd_file" "$mqtt_user" "$mqtt_pass"
                ...
                if grep -q "$mqtt_user:" "$passwd_file" 2>/dev/null; then
                  ...
                  print_msg "Failed to add user non-interactively." "$RED"
                fi
              else
                print_msg "Empty password provided, cannot proceed with non-interactive mode." "$RED"
              fi
            else
              # Interactive user addition (original method)
              print_msg "You will be prompted to enter a password for user '$mqtt_user'" "$YELLOW"
              print_msg "Please enter the same password twice when prompted" "$YELLOW"
...
install_workflow() {
  ...
  if [[ "$(uname -s)" == "Linux" ]]; then
    ...
    if command_exists apt-get; then
      ...
      print_msg "This may take some time, please be patient..." "$YELLOW"
      if sudo apt update && sudo apt upgrade -y; then
        print_msg "System update completed successfully" "$GREEN"
      else
        print_msg "System update encountered some issues, but we'll continue" "$YELLOW"
        print_msg "You may want to run 'sudo apt update && sudo apt upgrade -y' manually later" "$YELLOW"
      fi
    elif command_exists pacman; then
      print_msg "Running pacman update..." "$BLUE"
      ...
      print_msg "No supported package manager found, skipping system update" "$YELLOW"
    fi
  fi
